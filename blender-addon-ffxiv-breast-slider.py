...
def get_new_breast_size_vector(scale):
...
def update_breast_size_on_armature(armature, breastScalePercentage):
    if armature:
        bones = get_breast_bones(armature)

        for bone in bones:
            bone.scale = get_new_breast_size_vector(breastScalePercentage)
    


def do_update_breast_size( self, context ):
    update_breast_size_on_armature(context.scene.selectedArmature, self.breastScalePercentage)
